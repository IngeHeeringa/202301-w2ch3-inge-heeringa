...
const initializeGame = () => {
  ...
  gamePlaySelectors.letters.forEach((letter) =>
...
const startTimer = () => {
  const countdown = () => {
    ...
    let seconds = Math.floor(state.time - minutes * 60)
      .toString()
      .padStart(2, "0");
...
const findIndex = () => {
  if (
    letters.every(
      (letter) =>
        letter.state.gotRight || letter.state.gotWrong || letter.state.passed
    )
  ) {
    letters.forEach((letter) => {
      letter.state.passed = false;
  ...
  state.indexQuestion = letters.findIndex(
    (object) =>
      !object.state.gotRight && !object.state.gotWrong && !object.state.passed
  );
};
...
const renderQuestion = () => {
  ...
  gamePlaySelectors.letters[state.indexQuestion].classList.add(
    "letter_selected"
  );
...
const checkAnswer = (currentLetter) => {
  const currentQuestion = checkGameMode();
  const feedbackMessagesRightAnswer = [
    "Correct!",
    "You rock!",
    `That's right, ${usernameSelectors.username.innerText}!`,
    `${usernameSelectors.username.innerText} is on fire!`,
  ];
  const feedbackMessagesWrongAnswer = [
    `Oops! Did you mean "${currentQuestion.answer}"?`,
    `Nope! The answer is "${currentQuestion.answer}"`,
  ];
  ...
  switch (answer) {
    case currentQuestion.answer:
      ...
      gamePlaySelectors.answerFeedback.innerText =
        feedbackMessagesRightAnswer[
          Math.floor(Math.random() * feedbackMessagesRightAnswer.length)
        ];
      break;
    default:
      ...
      gamePlaySelectors.answerFeedback.innerText =
        feedbackMessagesWrongAnswer[
          Math.floor(Math.random() * feedbackMessagesWrongAnswer.length)
        ];
      break;
  }
};
...
gamePlaySelectors.submitAnswerButton.addEventListener("click", () => {
  ...
  if (!currentLetter.state.gotRight && !currentLetter.state.gotWrong) {
    ...
    gamePlaySelectors.letters[state.indexQuestion].classList.remove(
      "letter_selected"
    );
    if (
