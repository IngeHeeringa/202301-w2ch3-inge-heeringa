...
const extractOperator = (equationArr) => equationArr.match(operatorsInEquationArr).toString();

const formatDecimals = (result) => Number.isInteger(result) ? result : parseFloat(result.toFixed(3));

const calculate = (equation, operator, callback) => {    
    const firstNumsArr = equation.slice(0, equation.indexOf(operator));
    const secondNumsArr = equation.slice(equation.indexOf(operator) +1, equation.length);
...
clear.addEventListener('click', () => {
    let equationArr = formatEquationArr(equation);
    if (equationArr.match(operatorsInEquationArr) && !equationArr.includes(Infinity)) {
        let usedOperator = extractOperator(equationArr);
...
number.addEventListener('click', (e) => {
    ...
    if (current.value.length > 12) {
        ...
        current.value = current.value + e.target.value.replace('.', '');
        equation.pop();
    } else current.value = current.value + e.target.value;
});
});
...
operators.forEach((operator) => {
    ...
    if (equation.length === 0) {
        current.value = '0';
    } else if (operatorArr.includes(current.value) && operatorArr.includes(e.target.value)) {
